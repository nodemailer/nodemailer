...
export function detectMimeType(filename?: string | false): string {
    ...
    const extension = ((parsed.ext.substr(1) || parsed.name || '').split('?').shift() as string).trim().toLowerCase();
    const value = extensions.has(extension) ? (extensions.get(extension) as string | string[]) : defaultMimeType;
...
export function detectExtension(mimeType?: string | false): string {
    ...
    const rootType = (parts.shift() as string).trim();
    const subType = parts.join('/').trim();

    if (mimeTypes.has(rootType + '/' + subType)) {
        const value = mimeTypes.get(rootType + '/' + subType) as string | string[];
        if (Array.isArray(value)) {
            return value[0];
        }
        return value;
