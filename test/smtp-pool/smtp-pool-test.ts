...
describe('SMTP Pool Tests', () => {
    ...
    it('should send mail', (t, done) => {
        ...
        server.onData = function (stream: any, session: any, callback: any) {
            let chunks: Buffer[] = [];
    ...
    it('should send multiple mails', (t, done) => {
        ...
        function sendMessage(callback: () => void) {
            pool.send(
                {
                    data: {},
                    message: new MockBuilder(
        ...
        server.onData = function (stream: any, session: any, callback: any) {
            ...
            stream.on('data', () => {
                ...
                if (!killedConnections && sentMessages === total / 2) {
                    ...
                    callCallback = false;
                    server.connections.forEach((connection: any) => {
                    ...
                    setImmediate(() => {
                        for (let j = 5 - 1; j >= 0; j--) {
                            if (pool._connections[j] && pool._connections[j].connection) {
                                (pool._connections[j].connection._socket as net.Socket).emit('error', new Error('TESTERROR'));
                            }
                        }
                    });
                }
