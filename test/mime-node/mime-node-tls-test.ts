...
describe('MimeNode attachment fetch strict TLS', { timeout: 10000 }, () => {
    ...
    beforeEach((t, done) => {
        server = https.createServer(httpsOptions, (req, res) => {
            ...
            res.end('Hello HTTPS attachment');
        });
        server.listen(0, () => {
            href = 'https://localhost:' + (server.address() as AddressInfo).port;
            done();
        });
    });
    ...
    it('rejects a self-signed attachment host by default', (t, done) => {
        let mb = new MimeNode('text/plain').setContent({ href });
    ...
    it('allows a self-signed attachment host when the attachment opts out', (t, done) => {
        ...
        mb.build((err, msg) => {
            assert.ok(!err);
            assert.ok(/Hello HTTPS attachment/.test(msg.toString()));
            done();
        });
    });
});
