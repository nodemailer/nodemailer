...
describe('List-* header comment CRLF injection', () => {
    const send = (data: SendMailOptions) => {
        const transport = nodemailer.createTransport({ streamTransport: true, buffer: true });
        return new Promise<string>((resolve, reject) => {
            transport.sendMail(data, (err, info) => (err ? reject(err) : resolve((info.message as Buffer).toString('utf8'))));
        });
    };
    ...
    describe('List-* header comment escaping', () => {
        const listIdValue = async (comment: string) => {
            const raw = await send({
                from: 'sender@example.test',
                to: 'recipient@example.test',
                subject: 'list id',
        ...
        const listUrlValue = async (comment: string) => {
            const raw = await send({
                from: 'sender@example.test',
                to: 'recipient@example.test',
