...
//    report its own name and version without importing package.json (JSON
//    imports need different syntax in ES modules and CommonJS).
...
function generatePackageInfo() {
    ...
    const target = path.join(root, 'src', 'package-info.ts');
...
if (process.argv.includes('--package-info-only')) {
    generatePackageInfo();
} else {
    build();
}
