...
Nodemailer.prototype.validateSettings = function(callback){
    ...
    if(this.transport.options && this.transport.options.name === true){
        if(resolvedAddress){
            this.transport.options.name = resolvedAddress.hostname || resolvedAddress.address;
        }else{
            return resolver.resolve((function(err){
                if(resolvedAddress){
                    this.transport.options.name = resolvedAddress.hostname || resolvedAddress.address;
                }else{
                    this.transport.options.name = this.resolveHostname(this.options.name);
                }
                callback();
            }).bind(this));
        }   
...
var resolver = {
    ...
    _startResolving: function(){
        ...
        publicAddress({hostname: "localhost"},(function(err, data){
