    ...
    _getDsnRcptToArgs(rcpt) {
        ...
        if (this._envelope.dsn && this._supportedExtensions.includes('DSN')) {
            if (this._envelope.dsn.notify) {
				...
				// https://tools.ietf.org/html/rfc3461#page-8
                // if the ORCPT parameter is used, it MUST contain the same address as the RCPT TO address
                args.push('ORCPT=rfc822;' + shared.encodeXText(rcpt));
            }
        }
        ...
        if (!this._envelope.rcptQueue.length) {
            ...
            if (this._supportedExtensions.includes('PIPELINING')) {
                while (this._envelope.rcptQueue.length) {
                    ...
                    this._sendCommand('RCPT TO:<' + curRecipient + '>' + this._getDsnRcptToArgs(curRecipient));
                }
            } else {
                curRecipient = this._envelope.rcptQueue.shift();
                this._recipientQueue.push(curRecipient);
