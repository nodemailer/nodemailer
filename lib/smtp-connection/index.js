...
class SMTPConnection extends EventEmitter {
    ...
    _setEnvelope(envelope, callback) {
        ...
        this._envelope.from = {
            name: ((this._envelope.from && this._envelope.from.name) || null),
            address: ((this._envelope.from && this._envelope.from.address) || this._envelope.from || '').toString().trim(),
        };
        ...
        if (this._envelope.from.address && /[\r\n<>]/.test(this._envelope.from.address)) {
            return callback(this._formatError('Invalid sender ' + JSON.stringify(this._envelope.from.address), 'EENVELOPE', false, 'API'));
        }

        // check if the sender address uses only ASCII characters,
        // otherwise require usage of SMTPUTF8 extension
        if (/[\x80-\uFFFF]/.test(this._envelope.from.address)) {
            useSmtpUtf8 = true;
        }
        ...
        this._sendCommand('MAIL FROM:' + (this._envelope.from.name ? ('"' + this._envelope.from.name + '"') : '') + '<' + this._envelope.from.address + '>' + (args.length ? ' ' + args.join(' ') : ''));
    }
    ...
    _actionMAIL(str, callback) {
        ...
        if (Number(str.charAt(0)) !== 2) {
            if (this._usingSmtpUtf8 && /^550 /.test(str) && /[\x80-\uFFFF]/.test(this._envelope.from.address)) {
