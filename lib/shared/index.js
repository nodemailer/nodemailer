 ...
 */
module.exports.parseConnectionUrl = (input) => {
    let str = '';
    let extraOptions = {};
    ...
    if (str) {
        ...
        if (url.auth) {
            ...
            options.auth = { user: auth.shift(), pass: auth.join(':') };
        }
        ...
        Object.keys(url.query || {}).forEach(key => {
            ...
            // Handle tls.* nested options
            if (key.startsWith('tls.')) {
                lKey = key.substr(4);
                if (!options.tls) options.tls = {};
                obj = options.tls;
            } else if (key.includes('.')) {
                return; // ignore other nested keys
            }
    ...
    // Merge remaining options (extraOptions) without overwriting existing keys
