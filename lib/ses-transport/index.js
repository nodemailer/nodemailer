...
class SESTransport {
    ...
    _checkSendingRate() {
        ...
        for (let i = 0; i < this.rateMessages.length; i++) {
            if (this.rateMessages[i].ts > now - this.rateInterval) {
                break;
            }
            remove++;
        }
        ...
        let delay = Math.max(this.rateMessages[0].ts + 1001, now + 20);
    ...
    _send(mail, callback) {
        ...
        setImmediate(() => getRawMessage((err, raw) => {
            ...
            this.ses.sendRawEmail(sesMessage, (err, data) => {
                ...
                let region = this.ses.config && this.ses.config.region || 'us-west-1';
